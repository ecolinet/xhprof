...
class XHProfRuns_Default implements iXHProfRuns
{
    ...
    /**
     * This function gets runs based on passed parameters, column data as key, value as the value.
     * Values
     ...
     *            An associative array containing, at minimum, type, days, and limit
     ...
     *
     * @param string $run_id
     *            unique identifier for the run being requested
     * @param mixed $type
     ...
     *
     * @param array $data
     *            An associative array containing the limit you'd like to set for the queyr, as well as either c_url or url for the desired element.
     ...
     * average, min, max execution time, etc.
     * This information is pushed into the global namespace, which is horribly hacky.
     *
     * @param string $url
     * @param string $c_url
     * @return array
     ...
     * thereof.
     * Similar URLs should become identical. Consider:
    ...
    public function save_run($xhprof_data, $type, $run_id = null, $xhprof_details = null)
    {
        ...
        if (! isset($GLOBALS['_xhprof']['serializer']) || strtolower($GLOBALS['_xhprof']['serializer'] == 'php')) {
            ...
            if (isset($_xhprof['savepost']) && $_xhprof['savepost']) {
                ...
                $sql['post'] = $this->db->escape(serialize(array(
                    "Skipped" => "Post data omitted by rule"
                )));
            }
        } else {
            ...
            if (isset($_xhprof['savepost']) && $_xhprof['savepost']) {
                ...
                $sql['post'] = $this->db->escape(json_encode(array(
                    "Skipped" => "Post data omitted by rule"
                )));
            }
        }
        ...
        $sql['c_url'] = $this->db->escape($this->_urlSimilartor($_SERVER['REQUEST_URI']));
