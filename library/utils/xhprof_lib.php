...
/*
 * The list of possible metrics collected as part of XHProf that require inclusive/exclusive handling while reporting. @author Kannan
 */
...
/*
 * Get the list of metrics present in $xhprof_data as an array. @author Kannan
 */
...
function xhprof_compute_flat_info($raw_data, &$overall_totals)
{
    ...
    /*
     * initialize exclusive (self) metric value to inclusive metric value to start with. In the same pass, also add up the total number of function calls.
     */
...
function xhprof_compute_inclusive_times($raw_data)
{
    ...
    /*
     * First compute inclusive time for each function and total call count for each function across all parents the function is called from.
     */
    foreach ($raw_data as $parent_child => $info) {
        ...
        if ($parent == $child) {
            /*
             * XHProf PHP extension should never trigger this situation any more. Recursion is handled in the XHProf PHP extension by giving nested calls a unique recursion-depth appended name (for example, foo@1).
             */
 ...
 *
 * @param
 *            s array $params An array whose keys are the names
